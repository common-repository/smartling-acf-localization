...
class AcfOptionHelper
{
    ...
    {
        return \Smartling\Helpers\OptionHelper::get(self::ACF_OPTION_KEY_MAP, []);
    }
    ...
    {
        \Smartling\Helpers\OptionHelper::set(self::ACF_OPTION_KEY_MAP, $map);
    }
    ...
    {
        $value = \Smartling\Helpers\OptionHelper::get($this->getName(), false);
        if (false !== $value) {
            $this->setValue($value);
    ...
    {
        if (!\Smartling\Helpers\StringHelper::isNullOrEmpty($this->getName())) {
            if (null === $this->getPk()) {
                $this->setId($this->tryGetOptionId());
            }

            \Smartling\Helpers\OptionHelper::set($this->getName(), $this->getValue());
        }
    }
